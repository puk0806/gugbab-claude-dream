...
사용자 요청에 따라 아래 산출물 중 필요한 것을 생성합니다. 별도 지정이 없으면 5종 모두 생성합니다.

1. **테스트 계획서** — 범위, 전략, 시나리오, 우선순위
2. **E2E 테스트 시나리오** — 정상/예외/경계값 케이스
3. **적대적(악성 유저) E2E 시나리오** — 공격·오남용·권한 우회·이상 경로 (**항상 필수**)
4. **Playwright 테스트 코드** — 실행 가능한 E2E 코드 (정상 + 적대적 시나리오 모두 코드화)
5. **회귀 테스트 체크리스트** — 기존 기능 보호 검증 목록

> 3번(적대적 E2E)은 사용자가 "정상 흐름만"이라고 명시하지 않는 한 **생략하지 않는다.**
> 근거·체크리스트는 @.claude/rules/adversarial-testing.md 를 따른다.
...
## 단계 3.5: 적대적(악성 유저) E2E 시나리오 — 필수

정상·예외·경계 케이스를 넘어, **악성 유저가 시스템을 뚫으려고 시도하는 시나리오**를 별도로 작성한다.
"이런 요청이 들어오면 **차단·거부·안전 실패**하는가"를 검증한다. (근거: @.claude/rules/adversarial-testing.md)

대상에 아래 공격 표면이 있으면 해당 시나리오를 **반드시** 포함한다.

### 인증·인가
- ADV-AUTH-01: 비로그인으로 보호 페이지 직접 URL 접근 → 로그인 리다이렉트 / 401
- ADV-AUTH-02: 다른 유저의 리소스 ID로 조회·수정(IDOR) → 403
- ADV-AUTH-03: 만료·위조·타 유저 토큰으로 요청 → 거부
- ADV-AUTH-04: 일반 유저가 관리자 전용 액션 호출 → 거부

### 입력 인젝션
- ADV-INJ-01: 입력란에 `<script>`·`onerror=` XSS 페이로드 → 이스케이프·미실행
- ADV-INJ-02: 검색·필터에 `' OR 1=1 --` 등 SQL 인젝션 → 결과 유출 없음
- ADV-INJ-03: 경로·파일명에 `../` 조작 → 거부
- ADV-INJ-04: 초장문·특수문자·유니코드 입력 → 검증 후 거부

### 비즈니스 로직 오남용
- ADV-BIZ-01: 수량·가격·금액을 클라이언트에서 조작 후 제출 → 서버 재검증
- ADV-BIZ-02: 동일 요청 중복·재생 제출 → 멱등 처리
- ADV-BIZ-03: 결제·인증 단계 건너뛰고 완료 요청 → 상태 전이 검증
- ADV-BIZ-04: 짧은 시간 대량 요청(무차별) → rate limit·잠금

### 파일·요청 위조
- ADV-FILE-01: 위장 확장자·초대용량 파일 업로드 → 차단
- ADV-CSRF-01: 교차 출처에서 상태 변경 요청 → CSRF 토큰 검증

각 시나리오는 단계 3의 작성 형식(유형=적대적, 우선순위=P0/P1)을 따르며,
"공격이 성공하면 안 된다"를 Then으로 명시한다. 단계 4에서 Playwright 코드로 구현한다.
...
- **정상 흐름(happy path)만 담은 테스트 스위트 금지** — 적대적·이상 경로 시나리오 필수 (adversarial-test-guard 훅이 차단)
