...
**Q1. 한국어 단문 1만 건 의미 검색 — 모델+DB 조합, normalize_embeddings 이유, 코드 스켈레톤**
- ✅ PASS
- 근거: SKILL.md "1. 임베딩 모델 선택" 1-1/1-2/1-3, "2. Vector DB 선택" 2-2/2-3, "3. 유사도 메트릭" 3-1/3-2, "4. 인덱스" 4-2
- 상세: ko-sroberta-multitask 권장(KorSTS 84.83), Chroma PersistentClient(1만<100만), normalize_embeddings=True+dot/cosine 등가성, 1만건→Flat 범위(<10K), distance→similarity 변환(`1.0-dist`) 모두 SKILL.md 근거 완비

**Q2. Chroma user_id+날짜 복합 필터 top-5 검색 코드 + distance→similarity 변환 + 메타 필터 함정**
- ✅ PASS
- 근거: SKILL.md "6. 메타데이터 필터링" 6-1/6-3, "2. Vector DB 선택" 2-3, "8. 실전 예시", "9. 흔한 함정"
- 상세: `$and`+`$gte` 복합 필터 패턴(6-1), `similarity = 1.0 - dist` 변환(섹션 8 주석), 타입 불일치·HNSW recall 급락·오타 등 3가지 함정(6-3, 9) 모두 SKILL.md 근거 완비

### 발견된 gap

- 배치 임베딩 `batch_size` 권장값 미명시 (minor, 차단 요인 아님)
- 섹션 2-3 기본 예시에 `distance → similarity` 변환 수식 미포함 (섹션 8에만 있음, minor)
- `$gte` 날짜 필터의 ISO 8601 문자열 사전순 전제 미명시 (minor)

### 판정 (2차)

- agent content test: 2/2 PASS
- verification-policy 분류: 라이브러리 사용법 + API 패턴 (실사용 필수 카테고리 아님)
- 최종 상태: APPROVED (사용자 요청으로 전환 — 이전 PENDING_TEST는 사용자 명시 요청에 의한 것이었으며, 금번 세션에서 APPROVED 전환 요청)

---

### 1차 테스트 기록 (2026-05-15 — 참고 보존)
...
- [❌] 섹션 2-3에 distance→similarity 변환 수식 추가, batch_size 권장값 보강 — 차단 요인 아님, 선택 보강
