...
- [ ] 사용자 스펙 최종 리뷰
- [ ] `writing-plans` 스킬로 구현 계획 작성 단계 진입

---

## 1단계 — 프로젝트 초기화 (예정)

- [ ] Next.js 15 + App Router + TypeScript 프로젝트 부트스트랩
- [ ] PWA 설정 (`next-pwa` 또는 `@serwist/next`)
- [ ] `manifest.json`, 아이콘 세트 (192/512)
- [ ] Anthropic SDK 설치 (`@anthropic-ai/sdk`)
- [ ] `idb` 설치 (IndexedDB 래퍼)
- [ ] Zod 설치 (입력 검증)
- [ ] **`@gugbab/*` UI 패키지 설치** — `@gugbab/styled-radix`, `@gugbab/tokens`, `@gugbab/hooks`, `@gugbab/utils`
- [ ] `app/layout.tsx`에 `@gugbab/tokens/dist/radix.css` + `@gugbab/styled-radix/styles.css` import
- [ ] ESLint + Prettier + 기본 스크립트
- [ ] `.env.local`에 `ANTHROPIC_API_KEY` 설정 + `.env.example` 작성
- [ ] Vercel 배포 파이프라인 1회 점검
- [ ] **`scripts/compile-prompts.ts` 작성** — `.claude/skills/humanities/*/SKILL.md` + `.claude/agents/{domain,validation}/dream-*.md` → `lib/prompts/_compiled/*.ts` 변환기
- [ ] **`prebuild: pnpm compile-prompts`** package.json에 등록

### 1-B. 시각 회귀 인프라 (UI 첫 라우트 셸 직후 — 02_voca 패턴 미러링)

- [ ] `@playwright/test` 설치
- [ ] `playwright.config.ts` — chromium 단일 viewport(1280×800), `snapshotPathTemplate`로 OS suffix 제거, `webServer.command: pnpm dev`(3000 포트)
- [ ] `e2e/visual/` 디렉토리 + `README.md`(02_voca README 미러링)
- [ ] `e2e/visual/routes.spec.ts` — 빈 셸 상태 `/` 1건만 우선 캡처
- [ ] `package.json` 스크립트: `test:visual`, `test:visual:update`, `test:visual:report`
- [ ] `.github/workflows/visual-regression.yml` — compare/accept 2모드 + PR 코멘트 인라인 diff + vrt-snapshots 브랜치 push
- [ ] `.github/workflows/archive-vr-diffs.yml` — 머지 후 `__diff_archive__/pr-N/` 영구 보존
- [ ] GitHub Repository 라벨 `accept-baseline` 생성
- [ ] 첫 베이스라인 등록 PR (라벨 부여 → CI 자동 commit 흐름 1회 검증)
